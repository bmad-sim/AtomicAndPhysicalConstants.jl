        ...
        if typeof(AS) != Nothing
            ...
            if haskey(ATOMIC_SPECIES, AS)
                ...
                if iso ∉ keys(ATOMIC_SPECIES[AS].mass)
                    error(
                        """The isotope you specified is not available: Isotopes are specified by the atomic symbol and integer mass number.""",
                    )
                    return
                end
                if charge > ATOMIC_SPECIES[AS].Z
                    error(
                        f"You have specified a larger positive charge than the fully stripped {ATOMIC_SPECIES[AS].species_name} atom has, which is unphysical.",
                    )
                    return
                end
                mass = begin
